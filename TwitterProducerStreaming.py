...
class TwitterStreamListener(tweepy.StreamListener):
    ...
    def __init__(self, hashtags, twitter_ids, kafka_broker_uri, kafka_topic):
        ...
        try:
            self.producer = Producer({'bootstrap.servers': kafka_broker_uri})#, value_serializer=lambda m: json.dumps(m).encode('ascii'))
        except:
    ...
    def on_data(self, response):
        ...
        if user_mention["screen_name"] in self.hashtags:
            ...
            record = {user_mention["screen_name"] : response}
            # Writing Tweet to Kafa Topics into producer
            self.producer.produce(topic=self.kafka_topic, value=json.dumps(record).encode('ascii') )
            self.producer.flush()
